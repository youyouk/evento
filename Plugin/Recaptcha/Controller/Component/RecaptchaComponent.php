 ...
 */
	public function initialize(Controller $controller, $settings = array()) {
		...
		if (empty($this->privateKey)) {
			throw new Exception(__d('recaptcha', "You must set your private recaptcha key using Configure::write('Recaptcha.privateKey', 'your-key');!"));
		}
 ...
 */
	public function verify() {
		if (isset($this->Controller->request->data['recaptcha_challenge_field']) && 
			...
			$response = explode("\n", $response);
			if (empty($response[0])) {
				$this->error = __d('recaptcha', 'Invalid API response, please contact the site admin.');
				return false;
			}
			...
			if ($response[1] == 'incorrect-captcha-sol') {
				$this->error = __d('recaptcha', 'Incorrect captcha');
			} else {
