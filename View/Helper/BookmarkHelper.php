...
class BookmarkHelper extends Helper{
	...
	var $defaults = array('facebook', 'twitter');
	...
	var $bookmarks = array(
		...
		'google' => array(
				...
				'link' => 'https://www.google.com/bookmarks/mark?op=edit&bkmk={url}&title={title}',
				'icon'=> 'google.gif'
		...
		'twitter' => array(
			'name'  =>'Twitter',
			'link' => 'http://twitter.com/intent/tweet?url={url}&text={title}',
			'icon' => 'twitter.png'
			)
	);
	...
	function getBookMarks($pagetitle, $url = null, $sites = array()){
		...
		foreach($sites as $site){
			...
			if($site == 'twitter') {
				$tmp_pagetitle = $this->Text->truncate($pagetitle, 115);
			}
			...
			$iconLoc = $this->imgFolder . $this->bookmarks[$site]['icon'];
